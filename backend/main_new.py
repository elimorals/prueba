...
@app.post("/api/v1/reportes/generar", response_model=BaseResponse, tags=["Reportes"])
async def generar_reporte_ia(request: ReportGenerationRequest):
    """Generar reporte médico usando IA"""
...
async def procesar_mensaje_chat(request: ChatRequest):
    """Procesar mensaje del chat IA médico"""
    return await chat_service.procesar_mensaje_chat(request)
